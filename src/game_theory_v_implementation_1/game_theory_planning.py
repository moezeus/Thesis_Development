...
# NOTE : 
# 1. kendaraan violating constraint boundary road (perlu adjust reward)
# 2. setir saat maintain menjadi "nan", perlu di cek input output dari stanley control
# 3. batere kotak perlu ada cadangan
...
def calculate_stanley_control(X, Y, yaw, t_now, X_wp, Y_wp, dt_wp, v_wp, yaw_wp):
    ...
    # update, change the waypoint to follow based on trajectory waypoint
    if len(X_wp) == 1: 
        T_tr_idx = 0
    else : 
        T_tr_idx = (np.argmin(np.square(np.array(X_wp)[0:]-X)))
    ...
    if tracking_flag and not return_flag: 
        if iterate_waypoints < num_waypoints-2: 
            ...
            if Action_AV == 0:  #maintain
                ...
                if X_AV < sig_x[iterate_waypoints]:
                    iterate_waypoints += 1
                else: 
                    pass
