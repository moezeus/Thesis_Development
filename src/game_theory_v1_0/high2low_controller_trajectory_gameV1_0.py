...
from trajectory_gameV1_0 import Controller
# untuk menulis csv
import csv
...
while not rospy.is_shutdown():
    ...
    if (RECEIVED_STATE_VAL):
        ...
        if Generate_waypoints: 
            x_tr, y_tr, brake, pmap, pot_x, pot_y = Game_Theory_Controller._calculate_game_theory_action_V0(delta_t, state['x'],
                                                        state['y'], state['v'],
                                                        state['yaw'], state['t'])
            pot_x_filtered = []
            pot_y_filtered = []
            for i in range(len(pot_x)):
                if pot_x[i] < state['x']:
                    pot_x_filtered.append(pot_x[i])
                    pot_y_filtered.append(pot_y[i])
        ...
        num_waypoints = len(pot_x_filtered)
        duration_tr = float(1/num_waypoints)
        # duration_tr = 0.5
        ...
        if iterate_waypoints < num_waypoints-1: 
            ...
            control_param = np.block([[pot_x_filtered[iterate_waypoints], state['x']],
                                    [pot_y_filtered[iterate_waypoints], state['y']],
                                    [state['v'], state['yaw']],
                                    [delta_t, duration_tr], 
                                    [brake,brake]])
            ...
            pub_msg.potential_x = pot_x_filtered #x trajectory
            pub_msg.potential_y = pot_y_filtered #y trajectory
            # pub_msg.pmap = pmap #potential map

            Generate_waypoints = False
            if state['x']< pot_x_filtered[iterate_waypoints] : #check trajectory increment
                iterate_waypoints += 1
            if state['y']<params.lower_bound or state['y']>params.upper_bound : #off_road safety
                iterate_waypoints = 0
            ...
            if state['x']< pot_x_filtered[iterate_waypoints] : 
                iterate_waypoints = 0
            if state['y']<params.lower_bound or state['y']>params.upper_bound : 
                iterate_waypoints = 0
                Generate_waypoints = True
            # iterate_waypoints = 0
            pmap = []
        # pub_msg.pose.covariance[7] = pmap #potential map
