...
class Controller(object):
    ...
    def _update_error_trajectory(self, x, y, v, yaw, t, dt):
        ...
        self.x_target = x_next
        self.y_target = y_next
        y1 = y
        x1 = x
    ...
    def _calculate_trajectory_control_signal(self, dt, x, y, v, yaw, t):
        ...
        # self.y_transform = y + self._l*np.sin(self.yaw_wp)
        # self.x_transform = x + self._l*np.cos(self.yaw_wp)
        ...
        # self.v_ref = (self.u1*np.cos(self.yaw_wp)+self.u2*np.sin(self.yaw_wp))/self.prius_wheel_radius
        ...
        # cs_long = np.fmax(np.fmin(cs_long, 0.072), 0.)#1.), 0.) #*0.065
        ...
        # omega = (-self.u1*np.sin(self.yaw_wp))/self._l + (self.u2*np.cos(self.yaw_wp))/self._l
        ggg = 2*self._l*omega
        ...
        cs_lat = delta
        # cs_lat = max(min(delta, self._sat_lat_max), self._sat_lat_min)
        ...
        if abs(cs_lat)>0.1: #kendali kecepatan saat AV berbelok lebih dari 0.1 rad
            cs_handbrake = 0.5
        # bisa-bisa saja, namun kurang stabil saat finish
        # ditambah program check waypoint di akhir
        if len(self._waypoints) - self._closest_idx < int(stopping_dist/wp_interpoint_dist):
            cs_long = 0
            cs_handbrake = 1
        # looks good, tapi perlu di cek lagi untuk trajectory game theory
        ...
        # print(self._closest_idx, self.t_trajectory, t, dt)# self.e_dx, self.e_dy, c, cs_long, self._ev)
        # print(self.v_ref, v)
        # print(v, self.v_ref, self.e_dx, self.e_dy, dt, yaw, self.v_ref)
        print("v_ref: "+str(self.v_ref))
        print("xnow:"+str(x)+" xtar:"+str(self.x_target)+" edx:"+str(self.e_dx))
        print("x_trans: "+str(self.x_transform))
        print("ynow:"+str(y)+" ytar:"+str(self.y_target)+" edy:"+str(self.e_dy))
        print("y_trans: "+str(self.y_transform))

        return cs_long, cs_lat, cs_handbrake, self.x_target, self.y_target
