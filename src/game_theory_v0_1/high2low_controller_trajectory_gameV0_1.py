...
from trajectory_gameV0_1 import Controller
...
pub_msg.header.frame_id = 'agv_path_following_control_' + "game_theory"
...
while not rospy.is_shutdown():
    ...
    if (RECEIVED_STATE_VAL):
        ...
        x_tr, y_tr, v_tr, yaw_tr, brake = Game_Theory_Controller._calculate_game_theory_action_V0(delta_t, state['x'],
                                                        state['y'], state['v'],
                                                        state['yaw'], state['t'])
        
        # buat parameter control dalam numpy block
        control_param = np.block([[x_tr, state['x']],
                                 [y_tr, state['y']],
                                 [v_tr, state['v']],
                                 [yaw_tr, state['yaw']],
                                 [delta_t, state['t']], 
        ...
        pub_msg.pose.covariance[6] = x_tr #x target
        pub_msg.pose.covariance[7] = y_tr #y target
